...
namespace QD.ERP.Web.Pages
{
    public class DocumentViewerModel : PageModel
    {
        ...
        private XtraReport LoadReportFromDbOrFactory(string reportName, Func<XtraReport> factoryMethod)
        {
            using var dbContext = GetReportDbContext();
            using var cmd = dbContext.Database.GetDbConnection().CreateCommand();

            cmd.CommandText = "SELECT ReportXML FROM tbl90112ReportAttributes WHERE ReportName = @reportName";
            var p = cmd.CreateParameter();
            p.ParameterName = "@reportName";
            p.Value = reportName;
            cmd.Parameters.Add(p);

            dbContext.Database.OpenConnection();
            var savedLayoutObj = cmd.ExecuteScalar();

            if (savedLayoutObj != null && savedLayoutObj != DBNull.Value)
            {
                byte[] layoutBytes;

                if (savedLayoutObj is byte[] bytes)
                {
                    layoutBytes = bytes;
                }
                else if (savedLayoutObj is string xmlString)
                {
                    layoutBytes = System.Text.Encoding.UTF8.GetBytes(xmlString);
                }
                else
                {
                    throw new InvalidOperationException(
                        $"Unsupported ReportXML type: {savedLayoutObj.GetType().FullName}");
                }
...

        {
            ...
            else if (reportName == "AccountWithNarration")
            {
                ...
                Report = LoadReportFromDbOrFactory(reportName, () => new AccountWithNarration(
            ...
            else if (reportName == "AccountExportFromatReport")
            {
                ...
                Report = LoadReportFromDbOrFactory(reportName, () => new AccountExportFromatReport(
                    AccountId, FrmDate, ToDate, tenantName, companyName, companyAddress, logoImage,
                    companyNameAr, companyAddressAr, userName, _tenantDbContextHelper
                )
                    );
            ...
            else if (reportName == "AccountOrderbyVchNoWONarrationReport")
            {
                ...
                Report = LoadReportFromDbOrFactory(reportName, () => new AccountOrderbyVchNoWONarrationReport(
                    AccountId, FrmDate, ToDate, tenantName, companyName, companyAddress, logoImage,
                    companyNameAr, companyAddressAr, userName, _tenantDbContextHelper
                    )
                );
            ...
            else if (reportName == "AccountExportLandscapeReport")
            {
                ...
                Report = LoadReportFromDbOrFactory(reportName, () => new AccountExportLandscapeReport(
                    AccountId, FrmDate, ToDate, tenantName, companyName, companyAddress, logoImage,
                    companyNameAr, companyAddressAr, userName, _tenantDbContextHelper
                    )
                );
            ...
            else if (reportName == "AccountOrderByVoucherNo")
            {
                ...
                Report = LoadReportFromDbOrFactory(reportName, () => new AccountOrderByVoucherNo(
                    AccountId, FrmDate, ToDate, tenantName, companyName, companyAddress, logoImage,
                    companyNameAr, companyAddressAr, userName, _tenantDbContextHelper
                    )
                );
