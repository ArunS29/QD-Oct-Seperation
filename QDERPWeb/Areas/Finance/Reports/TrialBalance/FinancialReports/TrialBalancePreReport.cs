...
{
    public partial class TrialBalancePreReport : DevExpress.XtraReports.UI.XtraReport
    {
        private readonly TenantDbContextHelper _tenantDbContextHelper;

        public TrialBalancePreReport(DateTime frmDate,
                                  DateTime toDate,
                                  string tenantName,
                                  string company_Name,
                                  ...
                                  bool isUseEffectiveDate = true)
        {
            ...
            try
            {
                sqlDataSource1.ConnectionOptions.CommandTimeout = 120; // 👈 Add this line
                sqlDataSource1.Fill();
            }
            catch (Exception ex)
            {
        ...
        private void ConfigureSqlQuery()
        {
            if (_tenantDbContextHelper != null && _tenantDbContextHelper.TryGetTenantAndDbContext(out var tenant, out var _))
            {
                ...
                string fullTableName = $"{schemaName}.FinancialSummaryReport";

                var customQuery = new CustomSqlQuery
                {
                    Name = "FinancialSummaryReport",
                    Sql = $"SELECT * FROM {fullTableName} WHERE VoucherDate BETWEEN @StartDate AND @EndDate"
                };
