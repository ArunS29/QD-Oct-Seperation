    ...
    public partial class BillsPurchases : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        private void LoadReportData(string invoiceNo)
        {
            ...
            else
            {
                ...
                decimal totalAmount = Convert.ToDecimal(dt.Compute("SUM(TotalWtTaxAfterDiscAndDeductions)", ""));

                if (FindControl("xrLabel40", true) is XRLabel labelEnglish)
        ...
        private DataTable GetReportData(string invoiceNo)
        {
            ...
            try
            {
                if (_tenantDbContextHelper.TryGetTenantAndDbContext(out var tenant, out var _))
                {
                    ...
                    {
                        string query = "SELECT * FROM qry201_718VATPurchaseBillReport WHERE PurchaseVoucherNo = @InvoiceNo";
        ...
        {
            var fixedPictureBoxes = new[] { "xrPictureBox2", "xrPictureBox3", "xrPictureBox1", "xrPictureBox4" };
