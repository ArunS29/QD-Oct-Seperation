    ...
    public partial class TAXINVOICEWTDOCUMENTALLEVELDISCOUNTSS : XtraReport
    {
        ...
        private void SetWatermark()
        {
            if (!_isApproved)
            {
                ...
                this.Watermark.ForeColor = Color.FromArgb(80, 173, 216, 230); // 80 is alpha for transparency

                // Set the text direction to diagonal
                this.Watermark.TextDirection = DirectionMode.ForwardDiagonal;

                // Make the watermark appear behind report content
                this.Watermark.ShowBehind = true;

                // Do not stretch, just center
                this.Watermark.ImageTiling = false;
                this.Watermark.ImageViewMode = ImageViewMode.Stretch; // In case image used later
            }
        }
