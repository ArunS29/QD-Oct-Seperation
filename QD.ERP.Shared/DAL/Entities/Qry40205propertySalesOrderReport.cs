// <auto-generated> This file has been auto generated by EF Core Power Tools. </auto-generated>
#nullable disable
using System;
using System.Collections.Generic;

namespace QD.ERP.Shared.DAL.Entities;

public partial class Qry40205propertySalesOrderReport
{
    public string SalesOrderNo { get; set; }

    public DateTime? SalesOrderDate { get; set; }

    public string ClientCode { get; set; }

    public string ClientName { get; set; }

    public string Attention { get; set; }

    public string ClientContactNo { get; set; }

    public string ClientContactEmail { get; set; }

    public string ClientAddress { get; set; }

    public string ClientPono { get; set; }

    public DateTime? ClientPodate { get; set; }

    public string QuoteNo { get; set; }

    public DateTime? QuoteDate { get; set; }

    public string ClientRefNo { get; set; }

    public string Rfqcode { get; set; }

    public string PropertyTypeName { get; set; }

    public string PropertyAddlDescription { get; set; }

    public decimal? QuotedQuantity { get; set; }

    public string QuotedUom { get; set; }

    public string UnitType { get; set; }

    public string UnitDesc { get; set; }

    public decimal? LineOrderNo { get; set; }

    public decimal? UnitRate { get; set; }

    public bool? IsWithOperator { get; set; }

    public decimal? MobRate { get; set; }

    public decimal? DemobRate { get; set; }

    public decimal? LineTotalWoMobRate { get; set; }

    public decimal? LineTotalWtMobRate { get; set; }

    public string PaymentMode { get; set; }

    public decimal? AdvanceAmount { get; set; }

    public string CreditPeriod { get; set; }

    public decimal? CreditLimit { get; set; }

    public string DeliveryLocation { get; set; }

    public string DeliveryContactPerson { get; set; }

    public string DeliveryContactMobile { get; set; }

    public string DeliveryContactEmail { get; set; }

    public string OrderDuration { get; set; }

    public string MobChargesBy { get; set; }

    public string CustomsChargesBy { get; set; }

    public string ApprovedCafattch { get; set; }

    public string Project { get; set; }

    public string SalesPersonName { get; set; }

    public byte? SopreparedBySignatory { get; set; }

    public string SignatoryName { get; set; }

    public string SignatoryPosition { get; set; }

    public string SignatoryContact { get; set; }

    public string SignatoryEmail { get; set; }

    public string DeliveryRegion { get; set; }

    public string Expr1 { get; set; }

    public byte[] SignatureImage { get; set; }

    public DateTime? OrderExpiryDate { get; set; }

    public string VerifiedSignatoryName { get; set; }

    public string VerifiedSignatoryPosition { get; set; }

    public byte[] VerifiedSignatoryImage { get; set; }

    public string ApprovedSignatoryName { get; set; }

    public string ApprovedSignatoryPosition { get; set; }

    public byte[] ApprovedSignatoryImage { get; set; }

    public decimal? TotalCost { get; set; }
}