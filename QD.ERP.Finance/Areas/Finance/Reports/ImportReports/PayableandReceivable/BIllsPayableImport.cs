...
{
       public partial class BIllsPayableImport : DevExpress.XtraReports.UI.XtraReport
        {
            private readonly TenantDbContextHelper _tenantDbContextHelper;

            public BIllsPayableImport(
                string tenantName,
                string company_Name,
            ...
            {
                sqlDataSource4.Queries.Clear();

                var customQuery = new CustomSqlQuery
                {
                    Name = "tbl201SubLedgerPayablesMaster",
                    Sql = "SELECT * FROM tbl201SubLedgerPayablesMaster"
                };

                sqlDataSource4.Queries.Add(customQuery);

                if (_tenantDbContextHelper != null && _tenantDbContextHelper.TryGetTenantAndDbContext(out var tenant, out var _))
                {
                    sqlDataSource4.ConnectionParameters = new CustomStringConnectionParameters(tenant.ConnectionString);
                }
                else
                {
