...
{
    public partial class CostcenterGroupByDateImport : XtraReport
    {
        private readonly TenantDbContextHelper _tenantDbContextHelper;

        public CostcenterGroupByDateImport(
            string requestedBy,
            DateTime frmDate,
            DateTime toDate,
            string tenantName,
            ...
            Image logoImage, string companyNameAr, string companyAddressAr, string username, bool? useEffectiveDate)
        {
            ...
            if (FindControl("xrLabel27", true) is XRLabel effectiveDateLabel)
            {
        ...
        private void ConfigureSqlDataSource(string requestedBy, DateTime frmDate, DateTime toDate, bool? useEffectiveDate)
        {
            ...
            var selectQuery = new CustomSqlQuery()
            {
                ...
                Sql = $@"
            SELECT * FROM tbl20151CostAnalysisReport
            WHERE 
