...
{
    public partial class CostCenterGroupReportImport : XtraReport
    {
        private readonly TenantDbContextHelper _tenantDbContextHelper;

        public CostCenterGroupReportImport(
            string requestedBy,
            DateTime frmDate,
            DateTime toDate,
            string tenantName,
            ...
            Image logoImage, string companyNameAr, string companyAddressAr, string username)
        {
            ...
            AddSqlQueryWithMultitenantConnection(requestedBy, frmDate, toDate);
        }

        private void AddSqlQueryWithMultitenantConnection(string requestedBy, DateTime frmDate, DateTime toDate)
        {
            var selectQuery = new CustomSqlQuery()
            {
                Name = "tbl20151CostAnalysisReport",
                Sql = @"SELECT * FROM tbl20151CostAnalysisReport
                        WHERE 
            ...
            sqlDataSource2.Queries.Add(selectQuery);

            // Apply multitenant logic
            if (_tenantDbContextHelper != null && _tenantDbContextHelper.TryGetTenantAndDbContext(out var tenant, out var _))
            {
                var connectionParams = new CustomStringConnectionParameters(tenant.ConnectionString);
                sqlDataSource2.ConnectionParameters = connectionParams;
            }
            else
            {
            ...
            sqlDataSource2.Fill();
            var data = this.sqlDataSource2.Result["tbl20151CostAnalysisReport"];
            if (data == null || data.Count() == 0)
            {
                this.sqlDataSource2 = null;
                CreateNoDataLabel();
            }
        }
        ...
        {
            string[] pictureBoxNames = { "xrPictureBox2", "xrPictureBox3", "xrPictureBox4", "xrPictureBox5", "xrPictureBox6", "xrPictureBox7", "xrPictureBox8", "xrPictureBox9", "xrPictureBox10", "xrPictureBox11" };
