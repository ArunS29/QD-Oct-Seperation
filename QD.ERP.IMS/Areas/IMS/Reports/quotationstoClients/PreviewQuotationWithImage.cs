...
namespace QD.ERP.IMS.Areas.IMS.Reports.InventoryReports
{
    public partial class PreviewQuotationWithImage : DevExpress.XtraReports.UI.XtraReport
    {
            ...
            TenantDbContextHelper tenantDbContextHelper)
        {
            ...
            int[] labelNumbers = { 12, 45, 46, 47, 51, 49, 50, 56, 57, 55, 64, 65, 66, 88, 8 };
        ...
        private void LoadReportData(string quotationNo)
        {
            ...
            if (dt.Columns.Contains("currencyId") && dt.Rows[0]["currencyId"] != DBNull.Value)
            {
                ...
                {
                    foreach (string pictureBoxName in new[] { "xrPictureBox1", "xrPictureBox6", "xrPictureBox8", "xrPictureBox9", "xrPictureBox10" })
                    {
        ...
        private void ApplyConditionalVisibility(bool showSeal, bool showSignature, bool printLetterhead)
        {
            ...
            if (FindControl("xrPictureBox5", true) is XRPictureBox sealPicture)
                sealPicture.Visible = showSeal;

            // 🔹 Signature logic (xrPictureBox5, xrPictureBox6, xrPictureBox7)
            foreach (string signatureBox in new[] { "xrPictureBox2", "xrPictureBox3", "xrPictureBox4" })
            {
            ...
            if (FindControl("xrLabel94", true) is XRLabel lbl94)
                lbl94.Visible = printLetterhead;
            if (FindControl("xrLabel95", true) is XRLabel lbl95)
                lbl95.Visible = printLetterhead;


            if (FindControl("xrPictureBox11", true) is XRPictureBox logoBox)
                logoBox.Visible = printLetterhead;

            if (FindControl("xrLine6", true) is XRLine line3)
