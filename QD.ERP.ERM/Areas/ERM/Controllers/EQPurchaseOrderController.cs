...
namespace QD.ERP.ERM.Areas.ERM.Controllers
{
    ...
    public class EQPurchaseOrderController : Controller
    {
        ...
        private string GetAbbreviatedCompanyCode(string EquipPOAbbrv, int yearInDigit, DateTime invoiceDate, bool isResetByYear, ERPMasterWtDataContext dbContext)
        {
            try
            {
                var mprNumbers = dbContext.Tbl40126PropertyPomasters
                     .Where(d => !string.IsNullOrEmpty(d.Pono) && d.Pono.Length >= 4 &&
        ...
        [HttpDelete("{PropertyPochildNo}")]
        public async Task<IActionResult> DeletePropertyPoChild(long PropertyPochildNo)
        {
            if (PropertyPochildNo <= 0)
                return BadRequest(new { message = "PropertyPoChildNo is required", success = false });

            // ✅ Tenant-based DB helper (if you’re using multi-tenant architecture)
            if (!_tenantDbContextHelper.TryGetTenantAndDbContext(out Tenant tenant, out ERPMasterWtDataContext dbContext))
                return Unauthorized(new { message = "Invalid tenant", success = false });
            ...
            try
            {
                ...
                var child = await dbContext.Tbl40127PropertyPochildren
                    .FirstOrDefaultAsync(x => x.PropertyPochildNo == PropertyPochildNo);

                if (child == null)
                    return NotFound(new { message = "Child record not found", success = false });

                // 🗑️ Delete and save changes
                dbContext.Tbl40127PropertyPochildren.Remove(child);
                await dbContext.SaveChangesAsync();

                return Ok(new { message = "Child record deleted successfully", success = true });
            }
            catch (Exception ex)
            {
                // 🧯 Handle exceptions safely
                return StatusCode(500, new { message = "Error deleting child record", error = ex.Message, success = false });
            }
        }
