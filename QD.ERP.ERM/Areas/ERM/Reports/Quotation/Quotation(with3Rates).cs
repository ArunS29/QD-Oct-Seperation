...
{
    public partial class Quotation_with3Rates_ : DevExpress.XtraReports.UI.XtraReport

    {
...

        {
            ...
            // ✅ Hide entire panelAcceptance instead of individual labels
            if (FindControl("panelAcceptance", true) is XRPanel panelAcceptance)
            {
                panelAcceptance.Visible = clientAcknowledgement;
            }
        ...
        private void ApplyConditionalVisibility(bool showSeal, bool showSignature, bool printLetterhead)
        {
            ...
            if (FindControl("xrLabel107", true) is XRLabel lbl107)
                lbl107.Visible = printLetterhead;
            if (FindControl("xrLabel108", true) is XRLabel lbl108)
                lbl108.Visible = printLetterhead;
            if (FindControl("xrLabel91", true) is XRLabel lbl91)
                lbl91.Visible = printLetterhead;
            if (FindControl("xrLabel93", true) is XRLabel lbl93)
                lbl93.Visible = printLetterhead;
            if (FindControl("xrLabel92", true) is XRLabel lbl92)
                lbl92.Visible = printLetterhead;
        ...
        private void SetReportParameters(bool showSeal, bool showSignature, bool printLetterhead, bool pageBreakAfter, bool pageBreakBefore,string quotationNo, string tenantName, string companyName, Image logoImage, Image sealImage, string companyAddress,
        ...
        //        if (FindControl("xrLabel71", true) is XRLabel labelEnglish)
        ...
        private void LoadReportData(string quotationNo)
        {
            ...
            if (dt.Columns.Contains("currencyId") && dt.Rows[0]["currencyId"] != DBNull.Value)
            {
                ...
                {
                    foreach (string pictureBoxName in new[] { "xrPictureBox1", "xrPictureBox3", "xrPictureBox4", "xrPictureBox2" })
                    {
...

            {
...

                {
...

                    {
                        ...
                        string query = "SELECT * FROM qry40104RptQuotation WHERE  QuoteNo= @QuotationNo";
