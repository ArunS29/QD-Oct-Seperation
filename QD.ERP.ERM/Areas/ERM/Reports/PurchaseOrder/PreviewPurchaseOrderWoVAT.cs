...
namespace QD.ERP.Web.Areas.ERM.Reports.PurchaseOrder
{
    ...
    bool ShowSignatoryPositionOnly
)
        {
            ...
            if (FindControl("panelAcceptance", true) is XRPanel panel1)
        ...
        private void LoadReportData(string purchaseOrderNo)
        {
            ...
            this.DataSource = dt;
            this.DataMember = "";
            decimal totalAmount = Convert.ToDecimal(dt.Compute("SUM(TotalAmount)", ""));
            ...
            if (dt.Columns.Contains("currencyId") && dt.Rows[0]["currencyId"] != DBNull.Value)
            {
                ...
                {
                    foreach (string pictureBoxName in new[] { "xrPictureBox2", "xrPictureBox6", "xrPictureBox1" })
                    {
